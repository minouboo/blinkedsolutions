		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .premium-counter-area .premium-counter-icon .icon svg, {{WRAPPER}} .premium-counter-area .premium-counter-icon .icon svg *' => 'fill: {{VALUE}};',
				),
				'condition' => array(
					'premium_counter_icon_image' => array( 'icon', 'svg' ),
				),
		...
		if ( $draw_icon ) {
			$this->add_control(
				...
				array(
					...
					'selectors' => array(
						'{{WRAPPER}} .premium-counter-area .premium-counter-icon .icon svg *' => 'stroke: {{VALUE}};',
					),
				)
			);
			...
			function renderCounterIcon() {
				...
				if( 'icon' === iconType || 'svg' === iconType ) {
					...
					if( 'icon' === iconType ) {
						...
						var iconHTML = 'yes' !== settings.draw_svg ? elementor.helpers.renderIcon( view, settings.premium_counter_icon_updated, { 'aria-hidden': true }, 'i' , 'object' ) : false,
