		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .premium-title-header svg, {{WRAPPER}} .premium-title-header svg *' => 'fill: {{VALUE}}',
				),
				'condition' => array(
					'icon_type' => array( 'icon', 'svg' ),
				),
		...
		if ( $draw_icon ) {
			$this->add_control(
				...
				array(
					...
					'selectors' => array(
						'{{WRAPPER}} .premium-title-header svg *' => 'stroke: {{VALUE}};',
					),
				)
			);
			...
			if( 'yes' === settings.premium_title_icon_switcher ) {
				...
				if( 'icon' === iconType || 'svg' === iconType ) {
					...
					if( 'icon' === iconType ) {
						...
						var iconHTML = 'yes' !== settings.draw_svg ? elementor.helpers.renderIcon( view, settings.premium_title_icon_updated, { 'class': 'premium-title-icon', 'aria-hidden': true }, 'i' , 'object' ) : false,
