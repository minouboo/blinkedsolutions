		...
		$repeater->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} {{CURRENT_ITEM}}.premium-pricing-list-item svg, {{WRAPPER}} {{CURRENT_ITEM}}.premium-pricing-list-item svg *' => 'fill: {{VALUE}};',
				),
				'condition' => array(
					'icon_type' => 'icon',
				),
		...
		if ( $draw_icon ) {
			$repeater->add_control(
				...
				array(
					...
					'selectors' => array(
						'{{WRAPPER}} {{CURRENT_ITEM}}.premium-pricing-list-item svg *' => 'stroke: {{VALUE}};',
					),
				)
			);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .premium-pricing-icon-container svg, {{WRAPPER}} .premium-pricing-icon-container svg *'  => 'fill: {{VALUE}};',
				),
				'condition' => array(
					'icon_type' => 'icon',
				),
		...
		if ( $draw_icon ) {
			$this->add_control(
				...
				array(
					...
					'selectors' => array(
						'{{WRAPPER}} .premium-pricing-icon-container svg *' => 'stroke: {{VALUE}};',
					),
				)
			);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .premium-pricing-list-item svg, {{WRAPPER}} .premium-pricing-list-item svg *'  => 'fill: {{VALUE}};',
				),
			)
		);
			...
			<ul class="premium-pricing-list">
					...
					<li <?php echo wp_kses_post( $this->get_render_attribute_string( $key ) ); ?>>
							...
							<?php elseif ( 'svg' === $item['icon_type'] ) : ?>
								<?php echo $this->print_unescaped_setting( 'custom_svg', 'premium_fancy_text_list_items', $index ); ?>
								<?php
	...
	protected function content_template() {
		...
		if( 'yes' === settings.premium_pricing_table_icon_switcher ) {
			...
			if( 'icon' === iconType || 'svg' === iconType ) {
				...
				if( 'icon' === iconType ) {
					var iconHTML = 'yes' !== settings.draw_svg ? elementor.helpers.renderIcon( view, settings.premium_pricing_table_icon_selection_updated, { 'aria-hidden': true }, 'i' , 'object' ) : false,
			...
			<# if('yes' === settings.premium_pricing_table_list_switcher ) { #>
				<ul class="premium-pricing-list">
					<# _.each( settings.premium_fancy_text_list_items, function( item, index ) {
						...
						if( 'icon' === item.icon_type ) {
							var listIconHTML = 'yes' === item.draw_svg ? elementor.helpers.renderIcon( view, item.premium_pricing_list_item_icon_updated, { 'class': 'premium-pricing-feature-icon' , 'aria-hidden': true }, 'i' , 'object' ) : false,
